function [allCOCOdata,cocoDatastore,cocoNames] = cocoAPI(imagesDir,annotationFile,annoType)
% 功能：优雅的实现coco2014，coco2017数据集新接口
% 
% 输入：
%     imagesDir，string类型，输入COCO图像文件根目录
%     annotationFile，string类型，与之对应的标注json文件
%     annoType，string类型，标注类型，目前仅支持“instances”类别
% 输出：
...
%
% Example:
% cocoDir = './yourDataPath/coco2017/train2017/';
% annFile = './yourDataPath/coco2017/annotations_trainval2017/annotations/instances_train2017.json';
% [allCOCOdata,cocoDatastore,cocoNames] = cocoAPI(imagesDir,annotationFile)
...
% end
%
% MATLAB R2020b or higher, current only support "instances" type.
% author:cuixingxing
...
arguments
    ...
    annoType (1,1) {mustBeMember(annoType,["instances","captions","person_keypoints"])}="instances"
end
...
allAnnotations = rowfun(@groupFcn,allAnnotations,... % or findgroups/splitapply
    'GroupingVariable','image_id',...
    'OutputVariableName',["segments","iscrowd","bbox","category_id"]);

% remove unlabeled images
imagesTable(~ismember(imagesTable.image_id,allAnnotations.image_id),:)=[];
assert(all(imagesTable.image_id==allAnnotations.image_id));
allCOCOdata = [imagesTable,allAnnotations(:,2:end)];
